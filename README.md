...
> Аналіз виконується через **Google Gemini 2.5 Flash** — реальний AI з підтримкою vision.
...
├── lib/
│   └── gemini.ts            # Інтеграція з Google Gemini 2.5 Flash
├── components/
...
Потрібна змінна оточення:

```
VITE_GEMINI_API_KEY=ваш_ключ_з_aistudio.google.com
```

Створіть файл `.env.local` у корені проєкту з цим вмістом.

## Деплой

Проєкт задеплоєний на [Vercel](https://pan-pixel.vercel.app).  
Для оновлення — просто зробіть push у гілку `main`.
